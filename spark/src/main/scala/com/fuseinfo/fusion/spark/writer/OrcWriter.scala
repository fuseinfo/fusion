...
class OrcWriter(taskName:String, params:java.util.Map[String, AnyRef]) extends FileWriter(taskName, params) {
  ...
  override def applyWriter(writer: DataFrameWriter[Row], path: String): Unit = writer.orc(path)
